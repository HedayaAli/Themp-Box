...
void System(float diff){
  if (diff>0.3){
  digitalWrite(IN3cool,LOW);
  digitalWrite(IN4cool,LOW);
  analogWrite(ENBcool,0);
  cooler = 0;
  
  digitalWrite(IN1heat,HIGH);
  digitalWrite(IN2heat,LOW);
  heater = 1;
  if(diff<=1.5){
  int power = map(diff*10,3,15,60,255);
  analogWrite(ENAheat,power);
  heater = power;
  }
  else{
  analogWrite(ENAheat,255);
  heater = 255;
  }
 }

if (-0.5<diff && diff<0.3){
  digitalWrite(IN3cool,LOW);
  digitalWrite(IN4cool,LOW);
  analogWrite(ENBcool,0);
  cooler = 0;
...
if (diff<-0.5){
  ...
  digitalWrite(IN3cool,LOW);
  digitalWrite(IN4cool,HIGH);
  cooler = 1;

  if(diff<=2 && diff>-25.5){
  int power = map(-diff*10,5,20,60,255);
  analogWrite(ENBcool,power);
  cooler = power;
  }
  else{
  analogWrite(ENBcool,255);
  cooler = 255;
  }
  }
}
